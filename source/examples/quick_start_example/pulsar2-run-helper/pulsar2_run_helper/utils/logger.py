...
from .exception import Pulsar2RunHelperException
...
class Logger:
    ...
    def __init__(self, severity=INFO, colors=True, letter=True, timestamp=False, line_info=False):
        ...
        Args:
            ...
            letter (bool): Whether to prepend each logging message with a letter indicating
                it's severity. Defaults to True.
            timestamp (bool): Whether to include a timestamp in the logging output.
                Defaults to False.
            line_info (bool): Whether to include file and line number information in the
    ...
    def suppress(self, severity=CRITICAL):
        ...
        Args:
            severity (Logger.Severity): The severity to set the logger to.
    ...
    # message may be a callable which returns a message. This way, only if the message needs
    # to be logged is it ever generated.
            ...
            def apply_color(message):
                if self.colors:
                    try:
                        ...
                        self.colors = False
                        self.warning(
                            "colored module is not installed, will not use colors when logging. To enable "
                            "colors, please install the colored module: python3 -m pip install colored"
                        )
                        self.colors = True
                return message
    ...
    def critical(self, message):
        ...
        raise Pulsar2RunHelperException(message) from None  # Erase exception chain
